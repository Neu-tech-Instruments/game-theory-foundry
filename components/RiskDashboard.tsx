...
export const RiskDashboard: React.FC<RiskDashboardProps> = ({
    ...
    const allNodesPositioned = useMemo(() => {
        ...
        // First pass: compute the total vertical space required for each node's visible subtree
        const computeVisibleHeight = (nodes: any[]): number => {
            let totalH = 0;
            for (const n of nodes) {
                const isCollapsed = collapsedNodeIds.has(n.id);
                if (isCollapsed || !n.children || n.children.length === 0) {
                    n._visibleHeight = Y_SPACING;
                } else {
                    n._visibleHeight = computeVisibleHeight(n.children);
                }
                totalH += n._visibleHeight;
            }
            return totalH;
        };

        const rootHeight = computeVisibleHeight(selectedNetwork.genealogy);

        // Second pass: position nodes perfectly centered in their allocated vertical space
        const transform = (nodes: any[], layer: number, startY: number, parentX: number): PositionedNode[] => {
            let currentY = startY;
            return nodes.map((n) => {
                const h = n._visibleHeight || Y_SPACING;

                // Center node vertically in its allocated subtree space
                const nodeY = currentY + (h / 2) - (Y_SPACING / 2);
                const nodeX = parentX + LAYER_SPACING;

                // If user has pinned it, use their position, otherwise use computed tree layout
                const position = n.position || { x: nodeX, y: nodeY };
                const isCollapsed = collapsedNodeIds.has(n.id);
                ...
                if (n.children && !isCollapsed) {
                    children = transform(n.children, layer + 1, currentY, nodeX);
                }
        ...
        // Center the entire graph vertically around y=300, starting x at 400
        const positionedGenealogy = transform(selectedNetwork.genealogy, 0, 300 - (rootHeight / 2), 400 - LAYER_SPACING);
                    ...
                    {collapsedNodeIds.size > 0 && (
                        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-slate-900/80 backdrop-blur-md border border-slate-700 rounded-lg px-3 py-1.5 shadow-lg">
                            ...
                            <span className="text-[9px] font-black uppercase tracking-widest text-slate-300">
                                {allNodesPositioned.length} visible — click any node to toggle its sub-tier
                            </span>
                        </div>
                    )}
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <label className="flex flex-col items-center justify-center gap-4 p-8 border-2 border-dashed border-slate-200 rounded-2xl bg-white hover:border-blue-400 hover:bg-blue-50/30 transition-all cursor-pointer group">
                                            <input type="file" className="hidden" onChange={() => {
                                                setStep('VISION_SCAN'); setTimeout(() => {
...
export const NeuralDrillCard = React.forwardRef<HTMLDivElement, any>(({ comp, isActive, isEditMode, palette, onClick, onDelete, onDragStart }, ref) => {
    ...
    return (
        ...
        >
            ...
            {/* Icon Box */}
            <div className={`relative w-9 h-9 flex items-center justify-center rounded-sm bg-white shadow-sm border transition-all duration-300 
                ${isActive ? 'shadow-xl border-blue-500 scale-105' : 'border-slate-200'}`}
                style={{ borderColor: isActive ? undefined : color, backgroundColor: comp.isCollapsed ? '#f8f9fa' : 'white' }}
            >
                {/* Stack Effect for hidden content - implies depth */}
                {comp.hiddenChildCount > 0 && (
                    <>
                        <div className="absolute inset-0 translate-x-1 translate-y-1 -z-10 rounded-sm border border-slate-200 bg-slate-50/50" />
                        <div className="absolute inset-0 translate-x-0.5 translate-y-0.5 -z-10 rounded-sm border border-slate-200 bg-white" />
                    </>
                )}

                {(() => {
                    const Icon = CATEGORY_ICONS[nodeType] || Layers;
                    return <Icon className="w-4.5 h-4.5" style={{ color }} />;
                })()}

                {/* Expansion Indicator - Sleek side-pill that avoids label overlap */}
                {comp.hiddenChildCount > 0 && (
                    <div 
                        className="absolute left-full ml-3 top-1/2 -translate-y-1/2 px-2 py-0.5 rounded-full flex items-center gap-1 shadow-sm border border-dashed transition-all duration-300 hover:scale-110 whitespace-nowrap z-20"
                        style={{ 
            ...
            {/* Label Box - Revealed on Hover or Active with smoother transition */}
            <div className={`px-3 py-2 bg-white/95 backdrop-blur-md border rounded-md shadow-xl text-center transition-all duration-500 
                ${isActive ? 'border-blue-500 opacity-100 translate-y-0' : 'border-slate-200 opacity-0 group-hover/node:opacity-100 group-hover/node:translate-y-1'}`}
                style={{ position: 'absolute', top: '100%', left: '50%', transform: isActive ? 'translateX(-50%)' : 'translateX(-50%)' }}
            >
                <div className={`text-[9px] font-black tracking-[0.1em] truncate max-w-[120px] mb-1 ${nodeType === 'COMPONENT' ? 'uppercase text-[#1c1e21]' : 'capitalize text-slate-700'}`}>
                    {comp.name}
                </div>
                {comp.basePrice && (
                    <div className="text-[10px] font-black text-blue-600 font-mono flex items-center justify-center gap-1">
                ...
                {LEGEND_ITEMS.map((item) => {
                    ...
                    return (
                        <div
                            ...
                            className={`flex items-center gap-2.5 rounded-lg px-2 py-1.5 transition-all duration-200 ${isFilterable
                        ...
                        >
                            ...
                            <div className="flex flex-col flex-1">
                                <span className={`text-[10px] font-black uppercase tracking-tight leading-none transition-colors duration-200 ${isHidden ? 'text-slate-400 line-through decoration-slate-300' : 'text-slate-900'
                                    }`}>
                                    {item.label}
                                </span>
                            ...
                            {isFilterable && (
                                <div className={`w-3.5 h-3.5 rounded-full border-2 shrink-0 transition-all duration-200 ${isHidden
                        ...
                        <div key={i} className="absolute opacity-0 animate-[burst_3.5s_forwards]" style={{ animationDelay: `${0.2 + (i * 0.02)}s`, '--angle': `${angle}deg`, '--radius': `${radius}px` } as any}>
                            <div className="flex flex-col items-center gap-1">
                                <div className="w-10 h-10 bg-white border-2 border-slate-100 rounded-lg flex items-center justify-center shadow-sm">
                                    {(() => {
                                        const Icon = CATEGORY_ICONS[comp.category] || (comp.category === 'COMPONENT' ? Layers : Database);
                                        return <Icon className="w-4 h-4" style={{ color: comp.category === 'COMPONENT' ? '#3b82f6' : '#f76707' }} />;
                                    })()}
                                </div>
